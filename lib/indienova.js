import {fetchText, getEnv, NONE_EXIST_ERROR, normalizeImageUrl, page_parser} from "./common";
...
export async function gen_indienova(sid) {
  ...
  let fetch_init = {}
  if (getEnv('INDIENOVA_COOKIE')) {
    fetch_init = {headers: {"Cookie": getEnv('INDIENOVA_COOKIE')}}
  }

  let indienova_page_resp = await fetchText(`https://indienova.com/game/${sid}`, fetch_init)
    .catch(error => ({error}));
  if (indienova_page_resp.error) {
    return Object.assign(data, {
      error: `Unable to reach Indienova: ${indienova_page_resp.error.message}`
    });
  }
  let indienova_page_raw = indienova_page_resp.text;
  ...
  data["poster"] = data["cover"] = normalizeImageUrl($("div.cover-image img").attr("src")); // 提出封面链接
  data["chinese_title"] = $("title").text().split("|")[0].split("-")[0].trim(); // 提出标题部分

  let title_field = $("div.title-holder"); // 提取出副标部分
  data["another_title"] = title_field.find("h1 small").length > 0 ? title_field.find("h1 small").text().trim() : "";
  data["english_title"] = title_field.find("h1 span").length > 0 ? title_field.find("h1 span").text().trim() : "";
  ...
  data["intro"] = intro_field.find("div.bottommargin-sm").length > 0 ? intro_field.find("div.bottommargin-sm").text().trim() : "";
  ...
  data["rate"] = rating_field.length >= 4 ? `${rating_field[0]}:${rating_field[1]} / ${rating_field[2]}:${rating_field[3]}` : "";
  ...
  let cat = cat_field.length > 0 ? cat_field.text().trim().split("\n").map(x => x.trim()) : [];
  // 对cat进行去重并移除 "查看全部 +"
  ...
  data["level"] = level_field.length > 0 ? level_field.find("img").map(function () {
    return normalizeImageUrl($(this).attr("src"));
  }).get() : [];

  // 提取价格信息
  let price_fields = $("ul.db-stores");
  data["price"] = price_fields.length > 0 ? price_fields.find("li").map(function () {
