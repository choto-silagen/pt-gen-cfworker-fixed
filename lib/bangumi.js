...
export async function search_bangumi(query) {
  const search_result = await fetchJson(
  ...
  if (search_result.error) {
    return {
      error: `Unable to reach Bangumi API: ${search_result.error.message}`
    };
  }

  const bgm_search_json = search_result.json || {};
...
export async function gen_bangumi(sid) {
  ...
  if (subject_result.response.status >= 400) {
    return Object.assign(data, {
      error: `Bangumi API returned ${subject_result.response.status}.`
    });
  }
