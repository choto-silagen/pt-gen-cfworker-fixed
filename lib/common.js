...
export const AUTHOR = getEnv('AUTHOR') ? getEnv('AUTHOR') : "Rhilip";
const VERSION = "0.7.0";
...
async function fetchWithTimeout(url, init = {}) {
  const timeout_ms = Number(getEnv('FETCH_TIMEOUT_MS') || 15000);
  if (init.signal || typeof AbortController === "undefined") {
    return fetch(url, withDefaultHeaders(init));
  }
...
export async function restoreFromKV(cache_key) {
  /* global PT_GEN_STORE */
  if (globalThis['PT_GEN_STORE'] && typeof globalThis['PT_GEN_STORE'].get === "function") {
