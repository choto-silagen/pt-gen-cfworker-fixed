...
function imageList(items) {
  return (items || []).map(item => {
    ...
    return firstValue(item.src, item.url, (item.image || {}).src, (item.media || {}).src, (item.video || {}).poster);
  }).filter(Boolean);
}
...
export async function gen_epic(sid) {
  ...
  const epic_api_result = await fetchJson(`https://store-content.ak.epicgames.com/api/zh-CN/content/products/${sid}`)
    .catch(error => ({error}));
  if (epic_api_result.error) {
    return Object.assign(data, {
      error: `Unable to reach Epic Store API: ${epic_api_result.error.message}`
    });
  }

  const {response: epic_api_resp, json: epic_api_json} = epic_api_result;
  ...
  const carousel = page_data.carousel || {};
  ...
  data["screenshot"] = unique([
    ...imageList(gallery["galleryImages"] || gallery["images"] || gallery["media"]),
    ...imageList(carousel["items"])
  ]);
